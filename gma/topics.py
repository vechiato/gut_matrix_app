...
@login_required
def list_topics():
    # Query the TeamUser association table to get the team IDs where the current user is a member
    #user_team_ids = TeamUser.query.filter_by(user_id=current_user.id).with_entities(TeamUser.team_id).all()
    ...
    #team_topics = Topic.query.filter(Topic.team_id.in_(user_team_ids)).all()
...
#         return redirect(url_for('controllers.create_topic'))